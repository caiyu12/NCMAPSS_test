...
class poolRNN(nn.Module):
    def __init__(self, configs):
        super(poolRNN, self).__init__()
        ...
        self.seg_squeeze = nn.Linear(self.seg_len, 1)
    ...
    def encoder(self, x_enc):
        ...
        # (b, s, c) -> (b, c, s) for pooling
        x = x_enc.permute(0, 2, 1) # (b, c, s)
        # x = x.reshape(batch_size, self.enc_in, -1, self.seg_len)
        # x_pooled = self.seg_squeeze(x).squeeze(3)
