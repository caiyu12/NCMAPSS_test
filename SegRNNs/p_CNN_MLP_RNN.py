...
class p_CNN_MLP_RNN(nn.Module):
    """
    Paper link: https://arxiv.org/abs/2308.11200.pdf
    """

    def __init__(self, configs):
        super(p_CNN_MLP_RNN, self).__init__()
        ...
        self.channel_attn = nn.MultiheadAttention(batch_first=True,  embed_dim=self.d_model, num_heads=configs.n_heads)
    ...
    def encoder(self, x):
        ...
        # hn = hn.reshape(batch_size, self.enc_in, self.d_model)
        # hn, _ = self.channel_attn(hn,hn,hn)
        # hn = hn.reshape(1,-1, self.d_model)
        ...
        # 编码
        _, hy = self.rnn(pos_emb, hn)  # (1, batch_size * enc_in, d_model)
    ...
    def rul_prediction(self, x_enc):
        # 编码
        x_enc = self.encoder(x_enc) + x_enc  # (batch_size, seq_len, enc_in)

        # CNN 处理
        conv_out = self.conv_layers(x_enc.permute(0, 2, 1))  # (batch_size, num_filters * 2, seq_len // 4)

        # 展平
        flat_out = conv_out.reshape(conv_out.size(0), -1)  # (batch_size, num_filters * 2 * (seq_len // 4))
