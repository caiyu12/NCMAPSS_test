...
class SegRNN_pTSMixer(nn.Module):
    """
    Paper link: https://arxiv.org/abs/2308.11200.pdf
    """

    def __init__(self, configs):
        super(SegRNN_pTSMixer, self).__init__()
        ...
        if self.task_name == 'rul_prediction':
            ...
            self.squeeze = nn.Linear(self.seq_len, 1)
    ...
    def encoder(self, x):
        ...
        # x_pre_mix = x.view(batch_size, self.enc_in, self.seg_num_x, self.d_model)
        # x_mixed = self.channel_mixer(x_pre_mix)
        # x = x_mixed.reshape(batch_size * self.enc_in, self.seg_num_x, self.d_model)
    ...
    def rul_prediction(self, x_enc):
        ...
        for i in range(self.layer):
            enc_out = self.TSMixerBlock[i](x_enc)
        enc_out = enc_out + x_enc
        enc_out = self.encoder(enc_out) + enc_out
