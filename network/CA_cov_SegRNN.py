...
class CA_cov_SegRNN(nn.Module):
    """
    Paper link: https://arxiv.org/abs/2308.11200.pdf
    """

    def __init__(self, configs):
        super(CA_cov_SegRNN, self).__init__()
        ...
        # 添加交叉注意力层
        self.attention = nn.MultiheadAttention(embed_dim=self.d_model, num_heads=1)
        ...
        if self.task_name == 'rul_prediction':
            self.act = F.gelu
            self.dropout = nn.Dropout(configs.dropout)
            self.rul_projection = nn.Linear((configs.enc_in-4) * configs.seq_len, 1)
    ...
    def encoder(self, x):
        ...
        sensors = sensors.reshape(-1, self.seg_num_x, self.seg_len)  # [b*(c-4), seg_num_x, seg_len]
        sensors_emb = self.valueEmbedding(sensors)  # [b*(c-4), seg_num_x, d_model]
        ...
        # 对传感器嵌入进行 RNN 编码
        _, sensors_hn = self.rnn(sensors_emb)  # [1, b*(c-4), d_model]

        # 对协变量嵌入进行 RNN 编码
        _, covariates_hn = self.rnn(covariates_emb)  # [1, b*4, d_model]

        # 调整形状以适应交叉注意力
        sensors_hn = sensors_hn.view(batch_size, enc_in - 4, self.d_model)  # [b, c-4, d_model]
        covariates_hn = covariates_hn.view(batch_size, 4, self.d_model)    # [b, 4, d_model]

        # 转置以适应 nn.MultiheadAttention
        sensors_hn = sensors_hn.permute(1, 0, 2)  # [c-4, b, d_model]
        covariates_hn = covariates_hn.permute(1, 0, 2)  # [4, b, d_model]

        # 交叉注意力：传感器作为查询，协变量作为键和值
        attn_output, _ = self.attention(sensors_hn, covariates_hn, covariates_hn)  # [c-4, b, d_model]

        # 转回 [b, c-4, d_model]
        attn_output = attn_output.permute(1, 0, 2)

        # 结合注意力输出和传感器 hn
        combined_hn = sensors_hn.permute(1, 0, 2) + attn_output  # [b, c-4, d_model]

        # 后续处理：生成预测序列
        pos_emb = torch.cat([
            self.pos_emb.unsqueeze(0).repeat(enc_in - 4, 1, 1),
            self.channel_emb[4:].unsqueeze(1).repeat(1, self.seg_num_y, 1)
        ], dim=-1).view(-1, 1, self.d_model).repeat(batch_size, 1, 1)  # [b*(c-4), m, d]

        _, hy = self.rnn(pos_emb, combined_hn.repeat(1, 1, self.seg_num_y).view(1, -1, self.d_model))  # [1, b*(c-4)*m, d]
        for i in range(self.num_layers - 1):
            _, hy = self.rnn(pos_emb, hy)

        # 生成输出序列
        y = self.predict(hy).view(batch_size, enc_in - 4, self.pred_len)  # [b, c-4, s]
        y = y.permute(0, 2, 1)
